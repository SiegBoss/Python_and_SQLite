...
def insert():
    ...
    Button(windowInsert, text="Enviar", font=("Arial", 12), command = lambda: sendData(usuarioEntry.get(), emailEntry.get(), nameEntry.get(), lastnameEntry.get(), dateEntry.get())).grid(row=6, column=0, columnspan=2)
...
# Crear los botones | Create the buttons
buttonInsert = Button(window, text="Insertar", font=("Arial", 10),  command=lambda: insert())
buttonInsert.grid(row=0, column=0)
buttonRemove = Button(window, text="Eliminar", font=("Arial", 10), command=lambda: remove())
...
data["columns"]=("ID", "Usuario", "Email", "Nombre", "Apellido", "Fecha de Nacimiento")
